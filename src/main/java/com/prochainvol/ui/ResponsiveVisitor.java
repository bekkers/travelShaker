...
public class ResponsiveVisitor extends AbstractResponsiveVisitor implements
	...
	private static final Logger logger = Logger
			.getLogger(ResponsiveVisitor.class.getName());
	...
	public void visit(Filter filter) {
		for (FlightRecommendationPredicate<?> pred : filter.getPredicates()) {
			...
			case RELATIVE_MAX_PRICE:
				...
				generateLine("Fourchette de prix avant filtre", String.format(
						"[%.2f .. %.2f]", relativePricePredicate.getMinPrice(),
						relativePricePredicate.getMaxPrice()));
				generateLine(
						"Rapport max/min avant filtre",
						String.format("%.2f",
								relativePricePredicate.getMaxPrice()
										/ relativePricePredicate.getMinPrice()));
				generateLine(
						"Ecart max - min avant filtre",
						String.format("%.2f",
								relativePricePredicate.getMaxPrice()
										- relativePricePredicate.getMinPrice()));
				generateLine("Fourchette de prix après filtre", String.format(
						"[%.2f .. %.2f]", relativePricePredicate.getMinPrice(),
						relativePricePredicate.getRelativeMaxPrice()));
				generateLine(
						"Rapport max/min après filtre",
						String.format("%.2f",
								relativePricePredicate.getRelativeMaxPrice()
										/ relativePricePredicate.getMinPrice()));

				generateLine("taux", String.format("%.2f",
						relativePricePredicate.getValue()));
				break;
			case MAX_PRICE:
		...
		if (goingPaths.size() == 0) {
			...
			if (!flight.isReturnFlight()) {
				...
				for (Path goingFlight : goingPaths) {
					String value2 = String.format("%s %s%s",
							goingFlight.getCarrierName(),
							goingFlight.getCarrierIataCode(),
							goingFlight.getFlightNumber());
					String value3 = Constants.PROCHAINVOL_SHORT_DATE_TIME_FORMAT
							.format(goingFlight.getDateDecolage());
					generateLine(affichableSubSource + price, value2, value3,
							gridFormat);
				}
			} else {
				// return flights
				for (Path goingFlight : goingPaths) {
					// afficher l'aller
					String value2 = String.format("A: %s %s%s",
							goingFlight.getCarrierName(),
							goingFlight.getCarrierIataCode(),
							goingFlight.getFlightNumber());
					String value3 = Constants.PROCHAINVOL_SHORT_DATE_TIME_FORMAT
				...
				for (Path returnFlight : flight.getReturnPath()) {
					...
					// afficher le retour
					String value2 = String.format("R: %s %s%s",
							returnFlight.getCarrierName(),
							returnFlight.getCarrierIataCode(),
							returnFlight.getFlightNumber());
					String value3 = Constants.PROCHAINVOL_SHORT_DATE_TIME_FORMAT
							.format(returnFlight.getDateDecolage());
	...
	public void visit(RelativePricePredicate r) {
		generateLine(
				"Fourchette de prix avant filtre",
				String.format("[%.2f .. %.2f]", r.getMinPrice(),
						r.getMaxPrice()));
		generateLine("Rapport max/min avant filtre",
				String.format("%.2f", r.getMaxPrice() / r.getMinPrice()));
		generateLine("Ecart max - min avant filtre",
				String.format("%.2f", r.getMaxPrice() - r.getMinPrice()));
		generateLine(
				"Fourchette de prix après filtre",
				String.format("[%.2f .. %.2f]", r.getMinPrice(),
						r.getRelativeMaxPrice()));
		generateLine("Rapport max/min après filtre", String.format("%.2f",
				r.getRelativeMaxPrice() / r.getMinPrice()));
	}
	...
	public void visit(ReportUnit rapport) {
		...
		generateLine("Durée de la requête HTTP",
				Long.toString(rapport.getDuréeHttp()) + " ms");
		generateLine("Durée de l'analyse",
				Long.toString(rapport.getDuréeAnalyse()) + " ms");

		generateLine("Nb total de recommendations",
				Integer.toString(rapport.getNbRecommendationsRecues()));
		generateLine("Nb recommendations vol direct à l'aller",
				Integer.toString(rapport.getNbDirectAller()));
		generateLine("Nb recommendations vol direct au retour",
				Integer.toString(rapport.getNbDirectRetour()));
	...
	public void visit(RequestParams requestParams) throws ProchainvolException {
		...
		generateLine("Depart Date",
				Constants.PROCHAINVOL_SHORT_DATE_FORMAT.format(requestParams
						.getDepartureDate()));
		...
		if (travelType != TravelType.ONE_WAY) {
			generateLine("Return Date",
					Constants.PROCHAINVOL_SHORT_DATE_FORMAT
							.format(requestParams.getReturnDate()));
	...
	public void visit(RequestResult requestResult) {
		List<RequestResultUnit> requestResultUnits = requestResult
				.getRequestResultUnits();
		logger.trace(String.format("nb requestResultUnits = %d",
				requestResultUnits.size()));

		if (requestResultUnits.size() == 0) {
			generateH4Line("No response");
		} else {
			List<ReportUnit> rapports = requestResultUnits.stream()
					.map(r -> r.getReportUnit())
			...
			if (rapports.size() == 0) {
				...
				generateTr(
						strongText("Libellé"),
						rapports.stream()
								.map(r -> strongText(r.getProvider().toString()))
								.collect(Collectors.toList()));
				generateTr(
						"Durée de la requête HTTP",
						rapports.stream()
								.map(r -> Long.toString(r.getDuréeHttp())
										+ " ms").collect(Collectors.toList()));
				generateTr(
						"Durée de l'analyse",
						rapports.stream()
								.map(r -> Long.toString(r.getDuréeAnalyse())
										+ " ms").collect(Collectors.toList()));

				generateTr(
						"Nb recommendations vol direct à l'aller",
						rapports.stream()
								.map(r -> Integer.toString(r.getNbDirectAller()))
								.collect(Collectors.toList()));
				...
				generateTr(
						"Fourchette des prix avant filtrage",
						rapports.stream()
								.map(r -> String.format("[%.2f .. %.2f]",
										r.getMinPrice(), r.getMaxPrice()))
								.collect(Collectors.toList()));

				generateTr(
						"Taille du flux de recommendations avant filtrage",
						rapports.stream()
								.map(r -> Integer.toString(r
										.getNbRecommendationsRecues()))
								.collect(Collectors.toList()));

				// Rapport sur les filtrages

				if (requestResultUnits.size() > 0
						&& config.getCurrentFlightFilters().getPredicates()
								.size() > 0) {
					ProchainvolConfig config = requestResult
							.getProchainvolConfig();
					generateH4Line("Application des filtres dans l'ordre "
							+ config.getCurrentFlightFilters()
									.getPredicates()
									.stream()
									.map(r -> r.getFlightFilter().name()
											+ ((r instanceof RelativePricePredicate || r instanceof PricePredicate) ? "("
													+ r.getValue().toString()
													+ ")"
													: ((r instanceof MaxStopPredicate
															|| r instanceof MaxStopOnMovePredicate || r instanceof MaxStopOnReturnPredicate) ? "("
															+ r.getValue()
																	.toString()
															+ ")" : "")))
									.collect(
					...
					generateTr(
							strongText("libellés"),
							rapports.stream()
									.map(r -> strongText(r.getProvider().name()))
									.collect(Collectors.toList()));
					List<FlightRecommendationPredicate<?>> flightPredicates = requestResult
							.getProchainvolConfig().getCurrentFlightFilters()
							.getPredicates();
					for (int i = 0; i < flightPredicates.size(); i++) {
						FlightRecommendationPredicate<?> filter = flightPredicates
								.get(i);
				...
				requestResultUnits
						.forEach(r -> {
							List<FlightRecommendation> recommendations = r
									.getRecommendations();
							generateH4Line(r.getReportUnit()
									.getProvider().name());

							String errorMess = r.getReportUnit()
									.getErrorMess();
							if (errorMess != null && errorMess.length() > 0) {
								...
								try {
									...
									String msg = e.getCause() != null ? e
											.getCause().getMessage() : e
											.getMessage();
									generateLine("impossible de montrer les paramêtres de la requête : "
											+ msg);
								}
							}
	...
	public void visit(TravelplaceReaderReport rapportAirportReader) {
		...
		try {
			generateLine("Temps de chargement",
					Long.toString(rapportAirportReader.getDuree()) + " ms");
		} catch (ProchainvolException e) {
			generateLine("Temps de chargement", e.getMessage());
		}
		generateLine("Nb Airports, source",
				Integer.toString(rapportAirportReader.getSizeEntry()));
		...
		if (airports != null && airports.size() > 0) {
			...
			for (Entry<String, List<Travelplace>> entry : airports.entrySet()) {
				...
				for (Travelplace airport : entry.getValue()) {
					...
					i++;
					buf.append("{Iata = ").append(airport.getIata())
							.append(", lat = ").append(airport.getLatitude())
							.append(", lon = ").append(airport.getLongitude())
							.append("}");
				}
	...
	public void visitCountryMap(Map<String, String> map) {
		for (Map.Entry<String, String> entry : map.entrySet()) {
			...
			if (value.equals("_")) {
				generateLine(doc.createTextNode("_"),
						doc.createTextNode(entry.getKey()));
			} else {
